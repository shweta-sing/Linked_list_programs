...
list* detect_loop(list *start, int call)
{
    // call is being used to determine if detect_loop is being called from main or from any other function
    // if main is calling detect_loop, then print statements will get executed, else detect_loop would simply return where the loop is present
    list *current=NULL, *loop_ptr, *loop_start = NULL;
    hash *current_hash=NULL, *last_node = NULL, *hash_start = NULL;
...
int remove_loop(list *loop_found)
{
    list *loop;
    if(detect_loop(start, 1)==NULL)
        printf("\nNo loop present!");
    else
    {
        loop_found->next = NULL;
...
int main()
{
    ...
    while(choice!=4)
    {
        printf("\n\n1. Detect loop\t2.Remove loop\t3.print\t4.EXIT");
        ...
        switch(choice)
        {
            case 1:
                ...
                break;
            case 2:
                remove_loop(detect_loop(start, 1));
                break;
            case 3:
                print_list();
                break;
